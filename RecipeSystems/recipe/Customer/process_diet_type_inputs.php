     ...
     if(isset($_POST['predict_diet'])) {
      ...
      //  $cuisine_type = empty($_POST['cuisine_type_text']) ? 'american' : $_POST['cuisine_type_text'];
        ...
        // database  - mysql
         ...
         $predict_diet_type = '';
        if( !empty($predicted_diet_type_response) ) {
            $predict_diet_type = $predicted_diet_type_response['diet_type'];
            $diets_query = mysqli_query($conn, "SELECT * FROM diet  where LOWER(diet_name) = '$predict_diet_type' ");
