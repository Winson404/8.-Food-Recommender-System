...
<?php 
      ...
      if(isset($_GET['meal_plan']) && isset($_GET['cuisine_type'])) 
      ...
      $fetch = mysqli_query($conn, "SELECT * FROM recipe LEFT JOIN avg_rate ON recipe.recipe_Id=avg_rate.avg_recipe_Id WHERE recipe_diet_Id='$meal_plan' AND recipe_status='1' AND cuisine_type='$cuisine_type' ORDER BY avg_rate.avg_rate DESC");
      ...
      $meal = mysqli_query($conn, "SELECT * FROM recipe WHERE cuisine_type='$cuisine_type'");
      $row_meal = mysqli_fetch_array($meal);
      $cuisine = $row_meal['cuisine_type'];

      if(isset($_POST['Search'])) {
        $search = mysqli_real_escape_string($conn, $_POST['search']);
        $find = mysqli_query($conn, "SELECT * FROM recipe JOIN diet ON recipe.recipe_diet_Id=diet.diet_Id LEFT JOIN avg_rate ON recipe.recipe_Id=avg_rate.avg_recipe_Id WHERE recipe_name LIKE '%".$search."%' AND recipe_diet_Id='$meal_plan' AND cuisine_type='$cuisine_type' ORDER BY avg_rate.avg_rate DESC");
              ...
              <header class="hero">
                <div class="hero-container">
                  <div class="hero-text">
                    ...
                    <h4><span style="text-transform: uppercase;"><?php echo $cuisine; ?></span> Cuisine</h4>
                  </div>
                </div>
              </header>
                  ...
                  <?php while ($row =mysqli_fetch_array($fetch)) { ?>
                    <div class="item">
                      ...
                      <form action="process_save.php" method="POST">
                        ...
                        <div class="row" style="height:50px;" id="recipeName">
                        <h5 class="mt-2">
                          <?php 
