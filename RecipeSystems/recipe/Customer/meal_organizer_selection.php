...
<?php 
      if(isset($_GET['meal_plan']) && isset($_GET['day']) && isset($_GET['meal_time'])) 
      ...
      $fetch = mysqli_query($conn, "SELECT * FROM recipe LEFT JOIN avg_rate ON recipe.recipe_Id=avg_rate.avg_recipe_Id WHERE recipe_diet_Id='$meal_plan' AND recipe_status='1' AND meal_type='$meal_time' ORDER BY avg_rate.avg_rate DESC");
      ...
      if(isset($_POST['Search'])) {
        ...
        $find = mysqli_query($conn, "SELECT * FROM recipe JOIN diet ON recipe.recipe_diet_Id=diet.diet_Id LEFT JOIN avg_rate ON recipe.recipe_Id=avg_rate.avg_recipe_Id WHERE recipe_name LIKE '%".$search."%' AND recipe_diet_Id='$meal_plan'  AND meal_type='$meal_time' ORDER BY avg_rate.avg_rate DESC");
        ...
        ?>
         ...
         <!-- main -->
         <main class="page">
         <!-- <h3><?php echo $meal_time; ?> <i class="fa-solid fa-spoon"></i></h3>
                  ...
                  <?php while ($found =mysqli_fetch_array($find)) { ?>
                    <div class="item">
                      <form action="process_save.php" method="POST">
                        ...
                        <input type="hidden" class="form-control" value="<?php echo $day; ?>" name="day">
                        <input type="hidden" class="form-control" value="<?php echo $meal_time; ?>" name="meal_time">
                        <input type="hidden" class="form-control" value="<?php echo $_SESSION['user_id']; ?>" name="user_id">
                        ...
                        <button type="submit" class="btn btn-sm" name="add_Meal" style="width: auto; height: auto; background-color: #00b33c;"><b>Add to <?php echo $day; ?>'s Meal</b></button>
                  ...
                  <?php while ($row =mysqli_fetch_array($fetch)) { ?>
                    <div class="item">
                      <form action="process_save.php" method="POST">
                        ...
                        <button type="submit" class="btn btn-warning btn-sm" name="view_recipe" style="width: auto; height: auto;"><b>View recipe</b></button>
                        <button type="submit" class="btn btn-sm" id="meals" name="add_Meal" style="width: auto; height: auto; background-color: #00b33c;"><b>Add to <?php echo $day; ?>'s Meal</b></button>
                        </div></form>
                    </div>
                  <?php } ?>
